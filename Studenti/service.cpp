...
void Service::updateAll(int i)
{
    for(const auto&st:repo.getStudenti())
    {
        ...
        if(st.getVarsta()+i<=0) throw runtime_error("Sunt studenti cu varsta invalida;");
...
void Service::UndoAct() {
    if(to_undo.empty())
    {
        throw runtime_error{"Nu mai poti face undo!\n"};
    }
...
void Service::RedoAct() {
    if(to_redo.empty())
    {
        throw runtime_error{"Nu mai poti face redo!\n"};
